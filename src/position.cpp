...
bool Position::is_capture_possible(Side p) const {
  if (
      ...
      SW_move(allof[!p] & SW_move(discsof[p])) & ~all) return true;
...
bool Position::operator==(Position const& other) const {
  return discsof[WHITE] == other.discsof[WHITE] and
...
uint64_t Position::hash(Position const& p) {
  return p.discsof[WHITE] * 241235214125216341 +
...
Bitboard Position::moves_at(Square s, Side p, CaptureTag, DiscTag) const {
  ...
  return (NE_move(targets & NE_move(1ull << s)) & ~all) |
...
bool KingsPosition::operator==(KingsPosition const& other) const {
  return kings[WHITE] == other.kings[WHITE] and
